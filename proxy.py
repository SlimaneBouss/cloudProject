...
ping_vals = {}
nodes ={'54.173.238.64': 'MASTER' ,'34.207.77.56':'SLAVE1','54.224.215.235':'SLAVE2'}
for k in nodes.keys():
    ping_vals[k] = 0

#Connect to our master node
host_conn = pymysql.connect(host='54.173.238.64',user='localuser',password='local',db='sakila',port=3306,autocommit=True)
...
def execute_in_slave(cmd,clientsocket,ip,name) : 
    with SSHTunnelForwarder(ip, ssh_username='ubuntu',ssh_pkey='ec2-keypair.pem',remote_bind_address=('54.173.238.64',3306)) as tunnel:

        slave_conn = pymysql.connect(host='54.173.238.64',user='localuser',password='local',db='sakila',port=3306,autocommit=True)
